...
# CH 11 Exercise 5: dplyr grouped operations
...
biggest_avg_delay_month <- data.frame(
  flights %>% 
    group_by(month) %>% 
    summarize(avg_dep_delay = round(mean(dep_delay, na.rm = T), 2)) %>% 
...
print(biggest_avg_delay_month)

# If your above data frame contains just two columns (e.g., "month", and "delay"
# in that order), you can create a scatterplot by passing that data frame to the
# `plot()` function
delay_by_month <- data.frame(
  flights %>% 
    group_by(month) %>% 
    summarize(avg_dep_delay = round(mean(dep_delay, na.rm = T), 2)) %>% 
    select(month, avg_dep_delay)
