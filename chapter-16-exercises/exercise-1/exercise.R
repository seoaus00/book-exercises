...
# CH 16 Exercise 1: ggplot2 basics
...
ggplot(data = diamonds_sample) + 
  geom_point(
    mapping = aes(x = carat, y = cut, size = price)
  )

# Try coloring the above plot based on the diamond's price!
ggplot(data = diamonds_sample) + 
  geom_point(
    mapping = aes(x = carat, y = cut, size = price, color = price)
  )
...
ggplot(data = diamonds_sample, mapping = aes(x = carat, y = price)) + 
  ...
  #geom_smooth(mapping = color = "red")
  #geom_point(mapping = aes(y = cut), color = "red", alpha = .3)
