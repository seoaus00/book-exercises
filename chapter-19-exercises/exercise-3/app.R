...
# CH 19 Exercise 3: interactive Shiny apps
...
diamond_features <- colnames(diamonds)
...
my_ui <- fluidPage(
  includeCSS("shiny_css_demo.css"), # include the CSS
...
my_server <- function(input_list, output_list) {
  ...
  output_list$my_plot <- renderPlot({
    ...
    # if they select the checkbox, also include trendline geometry
